...
echo "AMO dir will be phase2_test_mv1_$AMOInstance"
if [ -d $AMOBaseDir/phase2_test_mv1_$AMOInstance ]; then
	rm -rf $AMOBaseDir/phase2_test_mv1_$AMOInstance phase2_test_mv1_$AMOInstance
fi
git -C $AMOBaseDir clone -b master --single-branch git@github.com:adini121/Addon-Tests.git phase2_test_mv1_$AMOInstance
git -C $AMOBaseDir/phase2_test_mv1_$AMOInstance checkout $CommitHash
git -C $AMOBaseDir/phase2_test_mv1_$AMOInstance checkout -b testing-branch 
}
...
if [ ! -f $REPORTS_DIR/AMOTests_mv1_"$AMOGitTag".log ];then
	touch $REPORTS_DIR/AMOTests_mv1_"$AMOGitTag".log
fi

if [ ! -f $REPORTS_DIR/AMO_BrowserIdList_mv1_"$AMOGitTag".log ];then
    touch $REPORTS_DIR/AMO_BrowserIdList_mv1_"$AMOGitTag".log
fi
}
...
DROP TABLE IF EXISTS sessionids_mv1_$AMOGitTag;
EOF
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
touch $AMOBaseDir/phase2_test_mv1_$AMOInstance/dbconftest.py
chmod 755 $AMOBaseDir/phase2_test_mv1_$AMOInstance/dbconftest.py 
cat > $AMOBaseDir/phase2_test_mv1_$AMOInstance/dbconftest.py << EOF
...
cp $CURRENT_DIR/cvariables.json $AMOBaseDir/phase2_test_mv1_$AMOInstance/variables.json
}

configureVirtualenv(){
echo "................................configuring AMO Virtualenv......................................."
cd $AMOBaseDir/phase2_test_mv1_$AMOInstance
...
--variables=variables.json --platform=MAC --destructive \
...
2>&1 | tee $REPORTS_DIR/AMOTests_mv1_"$AMOGitTag".log
}
