...
if [ ! -d $JENKINS_Test_DIR/Jenkins_ubuntu1.10_$Test_Instance ]; then
    git -C $JENKINS_Test_DIR clone -b ubuntu-1.10 --single-branch git@github.com:adini121/acceptance-test-harness.git Jenkins_ubuntu1.10_$Test_Instance
fi
...
gatherTestReports(){
	...
	if [ ! -f /home/nisal/Dropbox/TestResults/Jenkins/test_reports_"$JenkinsVersion".log ];then
			touch /home/nisal/Dropbox/TestResults/Jenkins/test_reports_"$JenkinsVersion".log
	fi
}
...
cd $JENKINS_Test_DIR/Jenkins_ubuntu1.10_$Test_Instance
TYPE=existing BROWSER=seleniumGrid JENKINS_URL=http://134.96.235.47:$startupPort/jenkins$JenkinsVersion/ mvn test 2>&1 | tee /home/nisal/Dropbox/TestResults/Jenkins/test_reports_"$JenkinsVersion".log
}
