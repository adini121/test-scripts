...
if [ ! -d $JENKINS_Test_DIR/Jenkins_master_ath_$TestInstance ]; then
    git -C $JENKINS_Test_DIR clone -b master --single-branch git@github.com:adini121/acceptance-test-harness.git Jenkins_master_ath_$TestInstance
else
    rm -rf $JENKINS_Test_DIR/Jenkins_master_ath_$TestInstance
    git -C $JENKINS_Test_DIR clone -b master --single-branch git@github.com:adini121/acceptance-test-harness.git Jenkins_master_ath_$TestInstance
fi
...
gatherTestReports(){
	...
	if [ ! -f /home/nisal/Dropbox/TestResults/Jenkins/master_ath_reports_"$JenkinsVersion".log ];then
			touch /home/nisal/Dropbox/TestResults/Jenkins/master_ath_reports_"$JenkinsVersion".log
	fi
}
...
cd $JENKINS_Test_DIR/Jenkins_master_ath_$TestInstance
TYPE=existing BROWSER=seleniumGrid JENKINS_URL=http://134.96.235.47:$startupPort/jenkins$JenkinsVersion/ mvn test 2>&1 | tee /home/nisal/Dropbox/TestResults/Jenkins/master_ath_reports_"$JenkinsVersion".log
}
