...
if [ ! -f $REPORTS_DIR/"$currentTime"_fireplaceTests_MV2_"$FireplaceGitTag".log ];then
	touch $REPORTS_DIR/"$currentTime"_fireplaceTests_MV2_"$FireplaceGitTag".log
fi

if [ ! -f $REPORTS_DIR/"$currentTime"_BrowserIdList_MV2_"$BedrockGitTag".log ];then
    touch $REPORTS_DIR/"$currentTime"_BrowserIdList_MV2_"$BedrockGitTag".log
fi
}
...
EOF
sed -i 's|test_session_ids|sessionids_MV2_'$FireplaceGitTag'|g' $FireplaceBaseDir/test_MV2_$FireplaceInstance/conftest.py
sed -i 's|/home/adi/python.txt|'$REPORTS_DIR'/'$currentTime'_BrowserIdList_MV2_'$FireplaceGitTag'.log|g' $FireplaceBaseDir/test_MV2_$FireplaceInstance/conftest.py

echo "................................configuring Fireplace test-properties......................................."
CURRENT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
cp $CURRENT_DIR/credentials.yaml $FireplaceBaseDir/test_MV2_$FireplaceInstance/credentials.yaml
}

configureVirtualenv(){
echo "................................configuring Fireplace Virtualenv......................................."
cd $FireplaceBaseDir/test_MV2_$FireplaceInstance
...
runFireplacetests(){
	#export DISPLAY=:0.0
	py.test -r=fsxXR --verbose --baseurl=http://134.96.235.47:$FireplacePort --host 134.96.235.159 --port 1235 --browsername=firefox --capability=browser:FIREFOX_30_WINDOWS_8_64 --capability=apikey:c717c5b3-a307-461e-84ea-1232d44cde89 --capability=email:test@testfabrik.com --capability=record:true --capability=extract:true --credentials=credentials.yaml --platform=MAC --destructive tests/desktop/consumer_pages/. 2>&1 | tee $REPORTS_DIR/"$currentTime"_fireplaceTests_MV2_"$FireplaceGitTag".log
}
